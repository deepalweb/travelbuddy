...
class AIPlanScreen extends StatefulWidget {
  ...
  final String? initialTravelerType;
  final List<String> initialInterests;
  final List<String> initialAvoid;
  ...
  const AIPlanScreen({
    ...
    this.initialBudget,
    this.initialTravelerType,
    this.initialInterests = const [],
    this.initialAvoid = const [],
...
class _AIPlanScreenState extends State<AIPlanScreen> {
  ...
  late String _travelerType;
  late List<String> _avoidPreferences;

  @override
  void initState() {
    super.initState();
    _travelerType = widget.initialTravelerType ?? 'couple';
    _avoidPreferences = List<String>.from(widget.initialAvoid);
    ...
    if (widget.initialDurationDays != null) {
      ...
      _endDate =
          _startDate!.add(Duration(days: widget.initialDurationDays! - 1));
    }
    if (widget.initialBudget != null) {
      _budget = switch (widget.initialBudget) {
        'budget' => 'Budget',
        'luxury' => 'Luxury',
        _ => 'Mid-Range',
      };
  ...
  Future<void> _loadPreferences() async {
    try {
      ...
      setState(() {
        _pace = prefs.getString('ai_pace') ?? _pace;
        if (widget.initialBudget == null) {
          _budget = prefs.getString('ai_budget') ?? _budget;
        }
        _travelerCount = prefs.getInt('ai_travelers') ?? 1;
        if (widget.initialInterests.isEmpty) {
              ...
              child: Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: 8),
                    Text(
                      'Create a realistic itinerary with route logic, confidence, and honest watchouts.',
                      style: TextStyle(
              ...
              child: Row(
                children: [
                  ...
                  IconButton(
                    ...
                    color:
                        _travelerCount < 10 ? Colors.deepPurple : Colors.grey,
                  ),
                ],
              ),
            ),
                  ...
                  Row(
                    children: [
                      _buildInfoChip(Icons.calendar_today,
                          '${_calculateDays()} days', Colors.blue),
                      SizedBox(width: 8),
                      _buildInfoChip(
                          Icons.people,
                          '$_travelerCount traveler${_travelerCount > 1 ? 's' : ''}',
                          Colors.orange),
                      SizedBox(width: 8),
                ...
                children: [
                  Expanded(
                    child: ElevatedButton.icon(
                      onPressed: _isSavingPlan ? null : _saveTripPlan,
                      icon: _isSavingPlan
                          ? const SizedBox(
                              width: 18,
                              height: 18,
  ...
  Widget _buildPlanHealthSummary() {
    final generated = _tripPlan?['generatedPlan'];
    if (generated is! Map) return const SizedBox.shrink();

    final score = (generated['planningConfidenceScore'] as num?)?.round();
    final realityCheck = generated['realityCheck'];
    final tripHealth = generated['tripHealth'];
    final summary =
        realityCheck is Map ? (realityCheck['summary'] ?? '').toString() : '';
    final warnings = realityCheck is Map && realityCheck['warnings'] is List
        ? (realityCheck['warnings'] as List)
            .map((item) => item.toString())
            .where((item) => item.isNotEmpty)
            .take(2)
            .toList()
        : <String>[];
    final overall = tripHealth is Map
        ? (tripHealth['overall'] ?? 'good').toString()
        : 'good';
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      'Trip health: ${overall.replaceAll('_', ' ')}',
                      style: const TextStyle(
                ...
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      TextButton.icon(
                        onPressed: () =>
                            _editActivity(_selectedDayIndex, index),
                ...
                Column(
                  ...
                  children: [
                    Text('Per Person',
                        style:
                            TextStyle(fontSize: 12, color: Colors.grey[600])),
                    Text('€${(totalCost / _travelerCount).toStringAsFixed(0)}',
                        style: TextStyle(
                            fontSize: 20,
                            fontWeight: FontWeight.bold,
  ...
  void _editActivity(int dayIndex, int activityIndex) {
    final activity = ((_tripPlan!['dailyPlans'] as List)[dayIndex]['activities']
        as List)[activityIndex];
    final titleController =
        TextEditingController(text: activity['activityTitle']);
  ...
  Future<void> _generateAIPlan() async {
    ...
    try {
      ...
      final tripPlan = await AITripService.generateTripPlan(
        ...
        durationDays: days,
        interests:
            _selectedInterests.isEmpty ? null : _selectedInterests.join(', '),
        interestList: _selectedInterests.toList(),
        avoid: _avoidPreferences,
        pace: _pace,
        budget: _budget,
  ...
  Future<void> _regeneratePlan() async {
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
            'This will create a new plan with the same preferences. Your current plan will be replaced.'),
        actions: [
          TextButton(
  ...
  Future<void> _saveTripPlan() async {
    if (_tripPlan == null || _isSavingPlan) return;
    ...
    try {
      ...
      final tripPlan = TripPlan(
        ...
        destination: (_tripPlan!['destination'] as String?) ??
            _destinationController.text,
        duration:
            (_tripPlan!['duration'] as String?) ?? '${_calculateDays()} days',
        introduction: (_tripPlan!['introduction'] as String?) ?? '',
        conclusion: (_tripPlan!['conclusion'] as String?) ?? 'Enjoy your trip!',
        totalEstimatedCost:
            totalCost > 0 ? '€${totalCost.toStringAsFixed(0)}' : '€0',
        estimatedWalkingDistance: '${_calculateDays() * 5} km',
        durationDays: _calculateDays(),
        metadata: {
          'generatedPlan': _tripPlan!['generatedPlan'],
        ...
        dailyPlans: (_tripPlan!['dailyPlans'] as List? ?? []).map((dayData) {
          return DailyTripPlan(
            ...
            activities:
                (dayData['activities'] as List? ?? []).map((activityData) {
              return ActivityDetail(
                timeOfDay: (activityData['timeOfDay'] as String?) ?? '09:00',
                activityTitle:
                    (activityData['activityTitle'] as String?) ?? 'Activity',
                ...
                estimatedDuration:
                    (activityData['estimatedDuration'] as String?) ?? '2 hours',
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('✅ Trip saved. Open it anytime from Trips.'),
          backgroundColor: Colors.green,
        ),
      );
