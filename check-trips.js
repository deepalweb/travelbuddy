...
async function checkTrips() {
  try {
    ...
    if (count > 0) {
      const trips = await TripPlan.find().select('tripTitle destination userId createdAt').limit(10);
      console.log('\n📋 Recent trips:');
      trips.forEach((trip, i) => {
        console.log(`${i + 1}. ${trip.tripTitle} - ${trip.destination} (User: ${trip.userId})`);
      });
    }
