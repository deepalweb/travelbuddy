...
if (process.env.NODE_ENV !== 'production') {
  app.use((req, res, next) => {
    ...
    res.header('Access-Control-Allow-Headers', 'Content-Type, Authorization, x-user-id, x-firebase-uid, x-user-tier, x-admin-secret, x-csrf-token, X-CSRF-Token, Accept, Origin, X-Requested-With, cache-control, Cache-Control, pragma, Pragma, expires, Expires');
    next();
  });
}
...
// Load directions endpoint
try {
  const directionsRouter = (await import('./routes/directions.js')).default;
  app.use('/api/routes', directionsRouter);
  console.log('✅ Directions endpoint loaded');
} catch (error) {
  console.error('❌ Failed to load directions endpoint:', error);
}
